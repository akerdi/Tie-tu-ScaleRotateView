...
- (NSString*)formatTime {
  ...
  if (nil == formatter) {
    ...
    formatter.dateFormat = TTLocalizedString(@"h:mm a", @"Date format:1:05");
...
- (NSString*)formatShortTime {
  ...
  if (diff < TT_DAY) {
    ...
    if (nil == formatter) {
      formatter = [[NSDateFormatter alloc] init];
      formatter.dateFormat = TTLocalizedString(@"M/d", @"Date format: 7/27");
      formatter.locale = TTCurrentLocale();
    }
    return [formatter stringFromDate:self];
  }
...
//////////////////////////////////////////////
-(NSString*)formatShortTimeOnleyMandD{
    ...
    NSString *  locationString=[[dateformatter stringFromDate:senddate] substringFromIndex:6];
    NSString *  selfTimeString =[[dateformatter stringFromDate:self] substringFromIndex:6];
    NSLog(@"locationString:%@",locationString);
    NSLog(@"%@",selfTimeString);
    
    
    NSTimeInterval now = abs([self timeIntervalSinceNow]);
    if([selfTimeString isEqualToString:locationString]){
    if(now<TT_MINUTE*5){
        return TTLocalizedString(@"刚刚", @"");
    }else if (now<TT_DAY){
        return [self formatTime];
    }else{
    static NSDateFormatter* formatter = nil;
    if (nil == formatter) {
        formatter = [[NSDateFormatter alloc] init];
        formatter.dateFormat = TTLocalizedString(@"M/d HH:mm", @"Date format:7月27日 1:05 pm");
        formatter.locale = TTCurrentLocale();
    }
    return [formatter stringFromDate:self];
    }
